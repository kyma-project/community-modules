        ...
        if (m.name == mod.name) {
          ...
          for (let v of m.versions) {
            if (mod.version == v.version) {
              if (found) {
                throw new Error(`Duplicated module version ${mod.name}:${mod.version}`)
              }
              found=true
          ...
          if (!found) {
            throw new Error(`Module version not found: ${mod.name}:${mod.version}, channel: ${ch.name}`)
          }
        }
      }
      await loadModule(mod)
    }
    fs.writeFileSync(`app/${ch.name}.json`, JSON.stringify(ch.modules, null, 2))
